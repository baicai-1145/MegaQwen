...
prefill_megakernel(
    const int* __restrict__ token_ids,
    const __nv_bfloat16* __restrict__ embed_weight,
    const __nv_bfloat16* __restrict__ audio_embeds,  // [audio_len, HIDDEN_SIZE] bf16 (optional)
    int audio_start_idx,
    int audio_len,
    ...
    // Phase 1.5: Patch audio span (optional). We expect a contiguous range.
    if (audio_embeds != nullptr && audio_len > 0) {
        int total = audio_len * HIDDEN_SIZE;
        for (int idx = (int)(blockIdx.x * blockDim.x + threadIdx.x);
             idx < total;
             idx += (int)(gridDim.x * blockDim.x)) {
            int r = idx / HIDDEN_SIZE;
            int c = idx - r * HIDDEN_SIZE;
            hidden[(audio_start_idx + r) * HIDDEN_SIZE + c] = __bfloat162float(audio_embeds[r * HIDDEN_SIZE + c]);
        }
    }
    grid.sync();
